 ...
 *  issues. This is the architecture the .3 describes.
 ...
 try {
  ...
  if (res.status === 413) {
   ...
   if (text.includes("EntityTooLarge") || text.includes("payload size")) {
    throw new Error(
     `File too large (${(file.size / 1024 / 1024).toFixed(1)} MB). The maximum supported size is 100 MB. Please compress the video or use a smaller file.`,
    );
   }
   throw new Error(
    `File too large (${(file.size / 1024 / 1024).toFixed(1)} MB). The maximum supported size is 100 MB.`,
   );
  }
