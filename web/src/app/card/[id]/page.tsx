...
export default function ProvenanceCardPage({ params }: { params: Promise<{ id: string }> }) {
 ...
 const handleDownloadManifest = useCallback(() => {
  if (!manifest?.manifest_json) return;
  const blob = new Blob([manifest.manifest_json], { type: 'application/json' });
  const url = URL.createObjectURL(blob);
  const a = document.createElement('a');
  a.href = url;
  a.download = `trace-manifest-${manifest.asset_id.slice(0, 8)}.json`;
  document.body.appendChild(a);
  a.click();
  document.body.removeChild(a);
  URL.revokeObjectURL(url);
  toast.success('Manifest downloaded', {
   description: `trace-manifest-${manifest.asset_id.slice(0, 8)}.json`,
  });
 }, [manifest]);

 if (loading) {
  return (
   <div className="min-h-screen flex items-center justify-center bg-gradient-to-b from-[#f7f8fa] to-[#eef2f7] dark:from-slate-950 dark:to-slate-900">
    <div className="flex flex-col items-center gap-3">
     <Loader2 className="h-8 w-8 text-[#2E5C8A] animate-spin" />
     <p className="text-sm text-slate-500 dark:text-slate-400">Loading provenance manifest…</p>
    </div>
   </div>
  );
 }
 ...
 if (error || !manifest) {
  return (
   <div className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-b from-[#f7f8fa] to-[#eef2f7] dark:from-slate-950 dark:to-slate-900 gap-4">
    <XCircle className="h-12 w-12 text-[#C0392B]" />
    <p className="text-slate-700 dark:text-slate-200 font-medium">{error || 'Asset not found'}</p>
    <a href="/">
 ...
 return (
  <div className="min-h-screen flex flex-col bg-gradient-to-b from-[#f7f8fa] to-[#eef2f7] dark:from-slate-950 dark:to-slate-900">
   <header className="border-b border-slate-200 dark:border-slate-800 bg-white/80 dark:bg-slate-950/80 backdrop-blur-md sticky top-0 z-20">
    <div className="mx-auto max-w-2xl px-4 py-3 flex items-center gap-3">
     <a
      href="/"
      className="text-slate-500 dark:text-slate-400 hover:text-[#1F3A5F] dark:hover:text-white transition-colors"
     ...
     <div className="flex items-center gap-2.5">
      ...
      <span className="font-bold text-[#1F3A5F] dark:text-slate-100">Trace Provenance Card</span>
     </div>
    </div>
   </header>
   ...
   <main className="flex-1 mx-auto w-full max-w-2xl px-4 py-8">
      ...
      <CardContent className="p-6 sm:p-8 space-y-6">
       ...
       <div className="flex items-start justify-between gap-4">
        <div className="min-w-0">
         <h1 className="text-xl font-bold text-[#1F3A5F] dark:text-slate-100 truncate">
          {fileName}
         </h1>
       ...
       <div className="flex flex-col items-center py-4">
        <motion.div
         ...
         className={`flex h-24 w-24 items-center justify-center rounded-full shadow-lg ${
          valid
           ? 'bg-[#2E8B57] shadow-emerald-200 dark:shadow-emerald-900/40'
           : 'bg-[#C0392B] shadow-red-200 dark:shadow-red-900/40'
         }`}
        >
         {valid ? (
       ...
       <div className="space-y-3">
        ...
        <div className="border-l-2 border-slate-200 dark:border-slate-700 pl-4 space-y-4">
         ...
         <TimelineItem
          ...
          value={
           manifest.assertions.find((a) => a.name === 'Generated using')?.value ||
       ...
       <div className="space-y-2">
        ...
        <div className="grid gap-1.5">
         {manifest.assertions.map((a, i) => (
          <div
           key={i}
           className="flex gap-3 text-sm py-1 border-b border-slate-50 dark:border-slate-800 last:border-0"
          >
           <span className="text-slate-500 dark:text-slate-400 min-w-[140px]">{a.name}</span>
        ...
        >
         <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-slate-900 dark:text-slate-100">
           Verification result:{' '}
           {verify.manifest.signature_valid ? '200 OK — signature valid' : 'signature invalid'}
          </span>
         </div>
       ...
       {verify && verify.verifications.length > 0 && (
        <div className="space-y-3">
         <h2 className="text-sm font-semibold text-slate-700 dark:text-slate-200 uppercase tracking-wide flex items-center gap-2">
          <History className="h-3.5 w-3.5" />
          Verification history
         </h2>
         <div className="border-l-2 border-slate-200 dark:border-slate-700 pl-4 space-y-2.5">
          {verify.verifications.map((v, i) => (
           <div key={i} className="relative">
            <div
             className={`absolute -left-[21px] top-1 flex h-3 w-3 items-center justify-center rounded-full bg-white dark:bg-slate-900 border-2 ${
            ...
            <div className="flex items-center justify-between gap-2">
             ...
             <Badge
              ...
              className={`text-xs ${
               v.result === 'valid'
                ? 'text-[#2E8B57] border-[#2E8B57]/30 bg-emerald-50 dark:bg-emerald-950/30'
                : 'text-[#C0392B] border-[#C0392B]/30 bg-red-50 dark:bg-red-950/30'
              }`}
             >
              {v.result}
             </Badge>
       ...
       <div className="space-y-2">
        <button
         ...
         className="w-full flex items-center justify-between text-sm font-semibold text-slate-700 dark:text-slate-200 uppercase tracking-wide hover:text-[#1F3A5F] dark:hover:text-white transition-colors py-1"
        >
        ...
        {showManifest && (
         ...
         >
          ...
          <pre className="trace-json max-h-72 overflow-auto rounded-lg bg-slate-950 dark:bg-black text-slate-100 p-4 text-xs font-mono leading-relaxed border border-slate-800">
           {prettyManifest}
          </pre>
         </motion.div>
        )}
       ...
       {/* Meta footer */}
       <div className="flex justify-between text-xs text-slate-400 dark:text-slate-500 pt-4 border-t border-slate-100 dark:border-slate-800">
        <span>
    ...
    <div className="flex items-center justify-center gap-4 mt-4 text-xs text-slate-400 dark:text-slate-500">
     <a
      ...
      className="inline-flex items-center gap-1 hover:text-[#2E5C8A] dark:hover:text-[#5b8ec0] transition-colors"
     >
...
function TimelineItem({
 ...
 return (
  <div className="relative">
   <div className="absolute -left-[21px] top-1 flex h-3 w-3 items-center justify-center rounded-full bg-white dark:bg-slate-900 border-2 border-[#2E5C8A]" />
