...
Architecture : the FastAPI service wraps the same Stamper
...
# A single shared SQLite handle for the whole process (
...
def create_app() -> FastAPI:
  ...
  # CORS: the verifier is *meant* to be publicly callable ( —
  ...
  # reviewers' browser can all reach the API.
  app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
  ...
  async def stamp_asset(
    ...
    # Resolve creator identity ( persistence).
  ...
  def verify_asset(asset_id: str, request: Request) -> dict:
    ...
    # Log the verification call ( Verification entity).
  ...
  # ------------------------------------------------------------------
  # GET /card/<asset_id> — HTML Provenance Card ()
  # ------------------------------------------------------------------
...
# Module-level app for `uvicorn tracekit.api.app:app` .
app = create_app()
