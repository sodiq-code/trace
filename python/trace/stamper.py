...
This is the core Day 1 deliverable (report Sec 31.1). The Stamper:
...
The stamp operation is deterministic (no LLM in the path — report Sec 21/24)
...
def _validate_metadata(model: str, prompt: str, creator: str) -> None:
    """Enforce the security caps from report Sec 26.2 (reject >2KB prompt)."""
    if len(prompt.encode("utf-8")) > config.MAX_PROMPT_BYTES:
        raise StamperError(
            f"prompt exceeds {config.MAX_PROMPT_BYTES} byte cap "
            "(report Sec 26.2 prompt-injection mitigation)"
        )
...
def _sanitize_basename(path: str | os.PathLike) -> str:
    """Defend against path traversal in uploaded filenames (report Sec 26.2)."""
