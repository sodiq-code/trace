"""FastAPI HTTP API tests — Validation Tests 2 & 3 (report Sec 32.2, 32.3).
...
def test_openapi_docs_available(client):
    """Report Sec 25.1: automatic OpenAPI at /docs (technical maturity signal)."""
    r = client.get("/docs")
    assert r.status_code == 200
...
def test_verify_endpoint_returns_correct_response(client):
    """Report Sec 32.2: 200 OK with {asset_id, file_hash,
...
def test_card_endpoint_renders_html_with_green_badge(client):
    """Report Sec 32.3: renders the source chain + green VALID badge."""
    stamp_r = client.post(
        "/v1/stamp",
        files={"file": ("thumb.png", _png_bytes(), "image/png")},
...
# ---------------------------------------------------------------------------
# End-to-end latency — Validation Test 4 (report Sec 32.4, <3s)
# ---------------------------------------------------------------------------

def test_end_to_end_under_3_seconds(client):
    """Report Sec 32.4: stamp → card → verify in under 3 seconds."""
