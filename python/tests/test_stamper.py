"""Stamper tests — the Day 1 Validation Test 1 (report Sec 32.1).
...
def test_prompt_length_cap(test_png):
    """Report Sec 26.2: reject prompts > 2KB."""
...
# ---------------------------------------------------------------------------
# THE Day 1 VALIDATION TEST (report Sec 32.1)
# ---------------------------------------------------------------------------
...
def test_stamp_then_verify_roundtrip_valid(trace_home, test_png):
    ...
    # --- Definition of done (report Sec 31.1) ---
    ...
    # Independent verification via the Verifier (report Sec 32.1: independent verifier).
    ...
    # The claim chain must be creator-readable (report Sec 29.1).
...
def test_stamp_latency_under_target(trace_home, test_png):
    """Report Sec 33.1: stamp latency target < 1s, acceptable < 2s."""
    stamper = Stamper(creator="t@e.st")
