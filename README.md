...
> **Provenance for every AI-generated creator asset.**
...
EU AI Act Article 50 makes AI-content labeling mandatory from August 2026.
Creators who use AI tools (Midjourney for thumbnails, ElevenLabs for voiceovers,
Runway for B-roll) have no way to comply at scale — the alternative is
$5K–$15K legal review per asset or risk of regulatory penalties.

**Trace is the compliance layer for the AI content era.** Drop in any
AI-generated asset and get a cryptographically-verifiable C2PA manifest, a
public Provenance Card URL, and a full source chain — in under one second.
The verification API is free for any platform, sponsor, or regulator to
integrate.

> **Product thesis:** Compliance is a workflow, not a feature. By moving
> provenance stamping to the moment of asset creation, Trace converts a
> $5K–$15K legal review into a sub-second library call.
...
- **Creator-readable.** Every C2PA assertion is rendered in plain English
  ("Generated by Midjourney v7") on the Provenance Card — never raw
  `std.c2pa.assertions.generator=midjourney-v7`.
- **Real cryptography.** ES256 (ECDSA P-256) signatures validated by the
  official c2pa-python Reader (`claimSignature.validated`,
  `timeStamp.validated`, `assertion.hashedURI.match`).
- **No file-size limit.** The production backend runs on Railway — uploads up
  to 100 MB+ are supported with real C2PA stamping.

---

## Live demo

The dashboard runs on Vercel with the Python backend on Railway:
...
| **Health check** | https://tranquil-bravery-production.up.railway.app/healthz |
...
**Why Railway + Vercel?** Vercel's Hobby plan caps Route Handlers at 4.5 MB.
By running the Python backend on Railway (no body limit) and having the
browser upload directly to it, files of any size stamp successfully. The
`NEXT_PUBLIC_TRACE_API_URL` env var tells the browser where the backend lives.
...
The E2E flow (Validation Test 4) is verified by agent-browser on every
deployment. The full path: dashboard drop → stamp → Provenance Card →
verify green checkmark.

```mermaid
flowchart TD
    A["1. Open trace-provenance.vercel.app"] --> B["2. Drop AI-generated file"]
    B --> C["3. Select AI model (auto-suggested)"]
    C --> D["4. Click 'Stamp N files'"]
    D --> E["5. Browser POSTs directly to Railway backend"]
    E --> F["6. c2pa-python stamps + signs ES256"]
    F --> G["7. Dashboard shows 'Provenance manifest attached — Valid'"]
    G --> H["8. Click 'View Provenance Card'"]
    H --> I["9. Card renders source chain + green VALID badge"]
    I --> J["10. Click 'Verify cryptographically'"]
    J --> K["11. Verifier returns 200 OK — signature valid"]
```
...
The C2PA manifest is tamper-evident: any modification to the asset bytes
invalidates the data hash; any modification to the manifest breaks the
signature. Re-running `trace verify` on a tampered file returns
...
builds the image and runs `trace init && trace serve --host 0.0.0.0 --port
